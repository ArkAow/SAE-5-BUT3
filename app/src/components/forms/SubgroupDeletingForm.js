...
const SubgroupDeletingForm = ({subgroup, deleteSubgroup, setDeletingSubgroup, isSaving}) => {
  const handleLeaving = () => {
      ...
      <div className={`fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 ${isSaving ? 'cursor-wait' : ''}`}>
        <div className="tooltip-centered-bigger min-w-[450px] w-1/3">
          <h2 className="text-2xl text-center mb-4">
            Voulez-vous vraiment <span className="text-red-500 font-bold">supprimer</span> le sous-groupe {subgroup.name}?
          </h2>
...
export default SubgroupDeletingForm;